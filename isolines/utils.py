...
def concave_hull(coords, knn, increase_knn=1):
    """
    Takes a point coordinates sequence and returns a concave hull Polygon based on k nearest neighbors heuristic
    :param coords: numpy array of 2d coordinates
    :param knn: int - the number of nearest neighbors this will determine how elaborate the polygon exterior will be
    :param increase_knn: int - a number that specifies by how many number to increase the algorithm calculation if there
     are too few neighbors
    ...
    if knn >= coords.shape[0]:
        knn = coords.shape[0] - 1

    c_bounds = np.hstack([coords, coords])

    rt_idx = rtree.index.Index()
    [rt_idx.insert(i[0], (i[1])) for i in enumerate(c_bounds)]

    start_knn = knn
    line = []
    min_y = coords[:, 1].min()
    min_y_idx = np.where(coords[:, 1] == min_y)
    first_idx = min_y_idx[0][0]
    line.append(coords[first_idx])
    first_point = coords[first_idx]
    idx = np.array(list(rt_idx.nearest(c_bounds[first_idx], knn)))
    idx = idx[idx != first_idx]
    points = coords[idx]
    dir_vecs = np.array([x - first_point for x in points])
    dir_vecs = np.array([i / np.linalg.norm(i) for i in dir_vecs])
    signs = np.array([np.sign(np.cross(np.array([.0, 1.]), i)) for i in dir_vecs])
    angles = np.array([np.degrees(np.arccos(np.clip(np.dot(np.array([.0, 1.]), i), -1.0, 1.0))) for i in dir_vecs])
    angles[signs == 1] = 360 - angles[signs == 1]
    angles = np.array([90 - x if x < 90 else 360 + 90 - x for x in angles])
    dists = np.array([np.linalg.norm(first_point - i) for i in points])
    max_dist = max(dists)
    s_points = np.array([angles, dists, idx, np.arange(0, idx.shape[0])]).T
    s_points = s_points[np.lexsort((-s_points[:, 1], s_points[:, 0]))]
    current_idx = int(s_points[0][2])
    current_point = coords[current_idx]
    new_dir = dir_vecs[int(s_points[0][3])]
    line.append(current_point)

    rt_idx.delete(first_idx, c_bounds[first_idx])
    ...
    while current_idx != first_idx:
        ...
        knn = start_knn
        while test_int == True:
            idx = np.array(list(rt_idx.nearest(c_bounds[current_idx], knn)))
            idx = idx[idx != current_idx]
            points = coords[idx]
            dir_vecs = np.array([x - coords[current_idx] for x in points])
            dir_vecs = np.array([i / np.linalg.norm(i) for i in dir_vecs])
            signs = np.array([np.sign(np.cross(new_dir, i)) for i in dir_vecs])
            angles = np.array([np.degrees(np.arccos(np.clip(np.dot(new_dir, i), -1.0, 1.0))) for i in dir_vecs])
            angles[signs == 1] = 360 - angles[signs == 1]
            angles = np.array([90 - x if x < 90 else 360 + 90 - x for x in angles])
            dists = np.array([np.linalg.norm(current_point - i) for i in points])
            s_points = np.array([angles, dists, idx, np.arange(0, idx.shape[0])]).T
            s_points = s_points[np.lexsort((-s_points[:, 1], s_points[:, 0]))]
            new_idx = int(s_points[0][2])
            ...
            if new_idx == first_idx:
                ...
                l = LineString(line)
                relate = l.relate(LineString([current_point,
                                              coords[new_idx]]))

                if relate in set(['1F1F00102', '101F00FF2', '001F001F2', '0F1F00102']):
                    ## this is to prevent the algorithm from getting stuck in a dead end loop
                    if count == knn + 1:
                        vertices = line

                        new_end_points = coords[new_idx]
                        end_point_dist = l.project(Point(new_end_points))
                        vertices = np.array(vertices + [new_end_points])
                        vertices_dists = np.array([l.project(Point(v)) for v in vertices])
                    ...
                    test_int = True
                    rt_idx.delete(new_idx, c_bounds[new_idx])


                else:
                    test_int = False
        rt_idx.delete(current_idx, c_bounds[current_idx])

        current_idx = new_idx
        current_point = coords[current_idx]

        if check_dist:
            if np.linalg.norm(first_point - current_point) >= max_dist:
                rt_idx.add(first_idx, c_bounds[first_idx])
    ...
    if len(line) < 4:
        ...
        return concave_hull(coords, knn + increase_knn, increase_knn)
    else:
        poly = Polygon(line)
        if poly.buffer(0.1).contains(MultiPoint(coords)) and poly.is_valid:
